...
public class PlayerController : MonoBehaviour
{
    ...
    float m_timeBeforeAllowedToShoot = 0.08f;
    int m_playerNumber;
    ...
    private void Awake()
    {
        AkSoundEngine.PostEvent("Player_Spawn", this.gameObject);
    ...
    private void Start()
    {
        ...
        m_playerNumber = gameObject.GetComponent<AgentManager>().m_playerID;
    }   
    ...
    void CheckIfGrounded()
    {
        ...
        if (m_grounded)
        {
            ...
            m_timeBeforeAllowedToShoot = 0.08f;
        ...
        else
        {
            ...
            m_timeBeforeAllowedToShoot -= Time.deltaTime;
        }        
    }
    ...
    void Shooting()
    {        
        if (m_timeBeforeAllowedToShoot <= 0 && m_isShooting)
        {
            if(m_currentAmmo > 0 && m_firerTimer <= 0)
            {
                ...
                if (!m_facingRight)
                {
                    GameObject bullet = Instantiate(m_bullet, new Vector2(m_shootingPoint.transform.position.x, m_shootingPoint.transform.position.y), Quaternion.identity/*m_spriteHolder.transform.rotation*/);
                    //bullet.GetComponent<Bullet>().ChangeDirection();
                    
                    bullet.GetComponent<Bullet>().ChangeDirection(-m_spriteHolder.transform.right, GetComponent<Collider2D>());
                    if (bullet.GetComponent<Rocket>() != null)
                    {
                        bullet.GetComponent<Rocket>().SetPlayerToIgnore(m_playerNumber);
                    }
                }
                else
                {
